...
async function addFieldsMigration() {
  console.log('Starting field migration for backend...');
...
addFieldsMigration()
  .then(() => {
    console.log('Backend field migration completed successfully');
    process.exit(0);
  })
  .catch((error) => {
    console.error('Backend field migration failed:', error);
    process.exit(1);
  }); 